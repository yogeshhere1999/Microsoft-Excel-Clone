...
$(document).ready(function () {
    ...
    for (let i = 1; i <= 100; i++) {
        ...
        let coloumn = $(`<div class="coloumn-name colId-${i}" id="colCod-${ans}  ">${ans}</div>`)
    ...
    for (let i = 1; i <= 100; i++) {
        ...
        for (let j = 1; j <= 100; j++) {
            ...
            let coloumn = $(`<div class = "input-cell" contenteditable="true" id = "row-${i}-col-${j}" data="code-${colCode}"></div>`);
            row.append(coloumn);
    ...
    function changeHeader(ele) {
        ...
        cellInfo["font-weight"].length > 0 ? $(".icon-bold").addClass("selected") : $(".icon-bold").removeClass("selected");
        cellInfo["font-style"].length > 0 ? $(".icon-italic").addClass("selected") : $(".icon-italic").removeClass("selected");
        cellInfo["text-decoration"].length > 0 ? $(".icon-underline").addClass("selected") : $(".icon-underline").removeClass("selected");
    ...
    function updateCell(property, value, defaultPossible) {
        ...
        $(".input-cell.selected").each(function () {
            ...
            if (cellData[selectedSheet] && cellData[selectedSheet][rowId]) {
    ...
    $(".icon-paste").click(function(){
        ...
        let rowDistence = rowId -selectedCells [0][0] ;
        let colDistence = colId -selectedCells [0][1] ;
        for(let cell of selectedCells){
            let newRowId = cell[0] + rowDistence;
            let newColId = cell[1] + colDistence;
            ...
            if(!cellData[selectedSheet][newRowId]){
                cellData[selectedSheet][newRowId] = {};
                 ...
                 if(cut){
                     ...
                     if(Object.keys(cellData[selectedSheet][cell[0]])==0){
        ...
        if ($(".sheet-options-modal").length == 0) {
            ...
            $(".sheet-delete").click(function () {
                if (Object.keys(cellData).length > 1) {
                    ...
                    let currSheetIndex = Object.keys(cellData).indexOf(selectSheet);
                    if (currSheetIndex == 0) {
